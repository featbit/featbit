...
| 8. Call analyze | Step 8 calls `POST /api/v1/envs/{envId}/experiments/{id}/runs/{runId}/analyze` with `forceFresh=true`. | `TEST_SCRIPT.md` Step 8 and expected analyze section. | Live runner asserts status `analyzing`, non-empty `inputData`, expected metric keys in `inputData`, and non-empty `analysisResult`. | Implemented; live evidence required |
