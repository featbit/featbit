...
$ContractTestOutput = Join-Path $RepoRoot "modules\back-end\artifacts\experiment-contract-tests"
...
if ($boundFlags.Count -ne 1 -or $boundFlags[0].key -ne "rd-checkout-treatment-fixed-v1") {
    throw "Manifest must define only the checkout treatment flag as bound to an experiment."
}
...
try {
    ...
    if (-not $SkipBackendContractTests) {
        Invoke-Checked "Backend experiment/stats/MCP OAuth contract tests" {
            dotnet test modules\back-end\tests\Application.IntegrationTests\Application.IntegrationTests.csproj `
                --filter "FullyQualifiedName~ExperimentControllerTests|FullyQualifiedName~ExperimentStatsControllerTests|FullyQualifiedName~McpOAuthControllerTests" `
                "-p:BaseOutputPath=$ContractTestOutput\"
        }
    }
}
finally {
