...
Verify that FeatBit REST APIs, feature flag mutation workflows, public evaluation and insight ingest, and the experiment analysis flow work together end to end in one environment.
...
- An experiment bound to a feature flag
- Primary metric and guardrail metric configuration records
- Public evaluation, insight ingest, stats query, and analyze verification results
...
- The authenticated user/token has permission to create project/env/flag/segment/experiment resources in the target workspace
- The target API, evaluation/event, and streaming services are reachable
- The local machine has .NET 10 SDK installed so it can execute the single-file C# runner
...
| Public evaluation/insight | `https://app-eval.featbit.co` |
| Legacy-compatible streaming option | `wss://app-eval.featbit.co` |
...
The preflight fetches the target OpenAPI document and verifies that the project, environment, feature flag, and segment management endpoints used by this script are advertised. It is console-only and does not create a report. Experimentation, experiment-stats, and MCP OAuth endpoints may be absent from the public SaaS Swagger; those endpoint contracts are verified through backend integration tests. The live scenario does not execute an MCP OAuth grant flow; offline verification covers `POST /api/v1/envs/{envId}/mcp/oauth/token` through `McpOAuthControllerTests`.
...
Live E2E runs use a deterministic data set id whose default is `fixed-v1`. The topology cycle normally clears the database before each run, so project, environment, segment, feature flag, user, and metric keys intentionally stay the same. Pass `--data-set-id` (or wrapper `-DataSetId`) only when an isolated rerun is needed; every generated key then uses that suffix.
...
| No. | Key | Type | Purpose |
| --- | --- | --- | --- |
| 1 | `rd-checkout-treatment-fixed-v1` | `boolean` | Experiment flag with control/treatment variants |
...
| rules | empty |
| references | All 9 non-experiment flags reference this segment; the experiment flag does not reference this segment |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `rd-checkout-treatment-fixed-v1` | `boolean` | `true` | `control=false`; `treatment=true` | No targeting rule | fallthrough `control` 50%, `treatment` 50%; `includedInExpt=true`; `exptIncludeAllTargets=true` | Bound to the experiment |
| 2 | `rd-banner-copy-fixed-v1` | `string` | `false` | `control-updated=control`; `candidate-1-updated=short`; `candidate-2-updated=direct`; `candidate-updated=candidate` | `User is in segment IsOneOf [segmentId]` serves first variation at 100% | fallthrough first variation 100%; `includedInExpt=true`; `exptIncludeAllTargets=true` | Not bound to an experiment |
...
| 10 | `rd-kill-switch-fixed-v1` | `boolean` | `false` | `control-updated=false`; `treatment-updated=true` | `User is in segment IsOneOf [segmentId]` serves first variation at 100% | fallthrough first variation 100%; `includedInExpt=true`; `exptIncludeAllTargets=true` | Not bound to an experiment |
...
After the main experiment run is analyzed, the runner creates independent experiments for the scenarios below. Each scenario gets its own feature flag, experiment id, run id, primary metric event, and the default preset observation window. No-layer scenarios intentionally omit `layerKey`, `layerId`, `sliceStart`, and `sliceEnd` from the run payload.

Scenario seed data is ingested through `POST /api/public/insight/track` with explicit timestamps inside the preset window. Dedicated scenario flags determine actual served variations through the same fixed rollout hashing used by FeatBit fallthrough rollout, and persisted experiment exposure and metric timestamps are fixed so manual UI analysis uses the same window every run.

For the default `--users 1500` run, exact raw assignment counts are determined by FeatBit rollout hashing. Sampled-control scenarios seed the full raw exposure population and use a stable sampling hash based on `flagKey`, served variation id, and user key, so repeated E2E runs produce the same analyzed metric counts.

| UI experiment name | Scenario id | Flag serving split | Run sampling | Layer eligibility | User key universe | Default final preset result |
| --- | --- | --- | --- | --- | --- | --- |
| `E2E S1 No layer 50/50 use all fixed-v1` | `balanced-50-50-use-all` | control 50%, treatment 50% | control 100%, treatment 100% | none | `e2e-balanced-50-50-use-all-*` | control raw `761`, analyzed `761`, conv `228`; treatment raw `739`, analyzed `739`, conv `333` |
| `E2E S2 No layer 90/10 sampled to 10/10 fixed-v1` | `skewed-90-10-to-10-10` | control 90%, treatment 10% | control 11.111111%, treatment 100% | none | `e2e-skewed-90-10-to-10-10-*` | control raw `1348`, analyzed `124`, conv `37`; treatment raw `152`, analyzed `152`, conv `68` |
| `E2E S3 Layer [0,30) 34/33/33 fixed-v1` | `layer-30-three-arm-34-33-33` | control 34%, treatment1 33%, treatment2 33% | all variants 100% | layer key `e2e-layer-30-three-arm-34-33-33-fixed-v1`, slice `[0,30)` | `e2e-layer-30-three-arm-34-33-33-*` | control raw `507`, analyzed `171`, conv `51`; treatment1 raw `485`, analyzed `146`, conv `66`; treatment2 raw `508`, analyzed `164`, conv `74` |
| `E2E S4 Layer [0,30) 80/20 sampled to 20/20 fixed-v1` | `layer-30-skewed-80-20-to-20-20` | control 80%, treatment 20% | control 22.222222%, treatment 100% | layer key `e2e-layer-30-skewed-80-20-to-20-20-fixed-v1`, slice `[0,30)` | `e2e-layer-30-skewed-80-20-to-20-20-*` | control raw `1213`, analyzed `75`, conv `23`; treatment raw `287`, analyzed `86`, conv `39` |
| `E2E S5 Same layer [30,60) companion fixed-v1` | `layer-exclusive-companion-30-60` | control 50%, treatment 50% | control 100%, treatment 100% | same layer key as `S3`, slice `[30,60)` | same user keys as `S3` | control raw `772`, analyzed `240`, conv `72`; treatment raw `728`, analyzed `183`, conv `82`; analyzed user overlap with `S3` must be `0` |
...
- The same-layer companion first probes an overlapping `[20,50)` slice and expects the backend to reject it, then saves the valid `[30,60)` slice and verifies zero analyzed-user overlap with the `[0,30)` scenario.
...
| Insight ingest | The runner posts preset-timestamp raw exposure events for every synthetic assignment and metric events for the full predesigned raw population to `POST /api/public/insight/track`. |
...
| --- | --- |
| run status | `analyzing` after `POST /api/v1/envs/{envId}/experiments/{id}/runs/{runId}/analyze` when samples are present |
...
| `analysisResult` | non-empty JSON generated by the experiment analyzer. The script does not hardcode a specific statistical posterior value, but it requires the analyzer to return structured output rather than an empty string/null. |
...
- Every flag contains the tags `e2e`, `experimentation`, and its own variation type
...
- The experiment flag keeps its `control` and `treatment` variation names/values so experiment analysis can bind the expected variants
...
- Every targeting update preserves the flag's current `disabledVariationId`, which is required by the current API contract
- Segment flag references include all 9 non-experiment flags and do not include the experiment flag
...
### 4. Verify Public Evaluation Without Insight Writes

Purpose: verify that flag configuration written by management APIs can be fetched and evaluated through the public evaluation API without creating exposure or metric insight events before the experiment run exists.

API:

| Action | Endpoint |
| --- | --- |
| Evaluate configured flags | `POST /api/public/featureflag/evaluate` |

Assertions:

- Public evaluation completes for all 10 flags for the two representative users
- Evaluation results contain variation identifiers and values
- A synthetic user included in the generated segment evaluates each non-experiment flag to the expected first variation
- Pre-seed evaluation does not write insight events
- Preset exposure and metric insight ingest happens later in Step 7 after the experiment run and metrics are configured

### 5. Create Experiment

Purpose: verify that experimentation APIs can create an experiment and bind it to the boolean treatment flag created earlier.

API:

| Action | Endpoint |
| --- | --- |
| Create experiment | `POST /api/v1/envs/{envId}/experiments` |
| Update experiment details | `PUT /api/v1/envs/{envId}/experiments/{id}` |
| Read experiment | `GET /api/v1/envs/{envId}/experiments/{id}` |
...
Purpose: verify environment-level experiment metric management, experiment metric selection, and multiple metric types.

API:

| Action | Endpoint |
| --- | --- |
| Look up metric by key | `GET /api/v1/envs/{envId}/experiment-metrics?key={key}` |
| Create missing metric | `POST /api/v1/envs/{envId}/experiment-metrics` |
| Update existing metric | `PUT /api/v1/envs/{envId}/experiment-metrics/{metricId}` |
| Create metric reuse experiment | `POST /api/v1/envs/{envId}/experiments` |
| Select experiment metrics | `PUT /api/v1/envs/{envId}/experiments/{id}/metrics` |
| Create metric reuse run | `POST /api/v1/envs/{envId}/experiments/{id}/runs` |
| Configure metric reuse run audience | `PUT /api/v1/envs/{envId}/experiments/{id}/runs/{runId}/audience` |
| Configure metric reuse run | `PUT /api/v1/envs/{envId}/experiments/{id}/runs/{runId}` |
| Configure metric reuse observation window | `PUT /api/v1/envs/{envId}/experiments/{id}/runs/{runId}/observation-window` |
| Read experiment | `GET /api/v1/envs/{envId}/experiments/{id}` |
...
Purpose: create an experiment run and automatically generate deterministic exposure and metric evidence through the public insight endpoint.

API:

| Action | Endpoint |
| --- | --- |
| Create run | `POST /api/v1/envs/{envId}/experiments/{id}/runs` |
| Update run | `PUT /api/v1/envs/{envId}/experiments/{id}/runs/{runId}` |
| Update experiment traffic assignment | `PUT /api/v1/envs/{envId}/experiments/{id}/runs/{runId}/audience` |
| Update observation window | `PUT /api/v1/envs/{envId}/experiments/{id}/runs/{runId}/observation-window` |
...
Purpose: verify that the experiment analysis endpoint can generate an analysis result from collected data.

API:

| Action | Endpoint |
| --- | --- |
| Analyze run | `POST /api/v1/envs/{envId}/experiments/{id}/runs/{runId}/analyze` |
| Read experiment | `GET /api/v1/envs/{envId}/experiments/{id}` |
...
- Create a new experiment bound to that scenario flag.
...
- Assign scenario-specific synthetic users deterministically, then ingest scenario-specific raw exposure and metric events with timestamps inside the preset window.
...
- Layer `0-30 + 34/33/33` analyzes only the eligible layer slice and keeps both treatment arms as separate observed variants.
- Layer `0-30 + 80/20 -> 20/20` samples 22.222222% of control and 100% of treatment inside the eligible layer slice, producing roughly balanced analysis evidence.
- Same-layer companion `[30,60)` reuses the same registered layer and same user key universe as the first layer scenario, rejects an overlapping `[20,50)` update, and has zero analyzed-user overlap with the `[0,30)` scenario.
...
| Experiment identity | Live report `Resources` section, or `GET /api/v1/envs/{envId}/experiments/{id}` | `experimentId` is non-empty; experiment is bound to `rd-checkout-treatment-fixed-v1` |
...
| No. | Key | Human check |
| --- | --- | --- |
| 1 | `rd-checkout-treatment-fixed-v1` | Enabled is `true`; variants are exactly `control=false` and `treatment=true`; there are no targeting rules; fallthrough splits `control` 50% and `treatment` 50%; `includedInExpt=true`; `exptIncludeAllTargets=true`; this is the only flag bound to an experiment. |
| 2 | `rd-banner-copy-fixed-v1` | Enabled is `false`; variants are `control-updated=control`, `candidate-1-updated=short`, `candidate-2-updated=direct`, `candidate-updated=candidate`; rule is `User is in segment IsOneOf [segmentId]`; rule and fallthrough serve the first variation 100%; experiment targeting flags are true; not bound to an experiment. |
| 3 | `rd-price-multiplier-fixed-v1` | Enabled is `true`; variants are `control-updated=1.0`, `candidate-1-updated=1.1`, `candidate-2-updated=1.2`, `candidate-updated=2.5`; rule is `User is in segment IsOneOf [segmentId]`; rule and fallthrough serve the first variation 100%; experiment targeting flags are true; not bound to an experiment. |
| 4 | `rd-checkout-config-fixed-v1` | Enabled is `false`; variants are `control-updated={"mode":"control","limit":5}`, `candidate-1-updated={"mode":"treatment","limit":10}`, `candidate-updated={"mode":"candidate","limit":15}`; rule is `User is in segment IsOneOf [segmentId]`; rule and fallthrough serve the first variation 100%; experiment targeting flags are true; not bound to an experiment. |
| 5 | `rd-onboarding-flow-fixed-v1` | Enabled is `true`; variants are `control-updated=classic`, `candidate-1-updated=guided`, `candidate-2-updated=compact`, `candidate-updated=candidate`; rule is `User is in segment IsOneOf [segmentId]`; rule and fallthrough serve the first variation 100%; experiment targeting flags are true; not bound to an experiment. |
| 6 | `rd-risk-threshold-fixed-v1` | Enabled is `false`; variants are `control-updated=10`, `candidate-1-updated=25`, `candidate-2-updated=50`, `candidate-updated=2.5`; rule is `User is in segment IsOneOf [segmentId]`; rule and fallthrough serve the first variation 100%; experiment targeting flags are true; not bound to an experiment. |
| 7 | `rd-ai-assistant-route-fixed-v1` | Enabled is `true`; variants are `control-updated=off`, `candidate-1-updated=gpt-4.1-mini`, `candidate-2-updated=gpt-4.1`, `candidate-updated=candidate`; rule is `User is in segment IsOneOf [segmentId]`; rule and fallthrough serve the first variation 100%; experiment targeting flags are true; not bound to an experiment. |
| 8 | `rd-notification-style-fixed-v1` | Enabled is `false`; variants are `control-updated=quiet`, `candidate-1-updated=badge`, `candidate-2-updated=toast`, `candidate-updated=candidate`; rule is `User is in segment IsOneOf [segmentId]`; rule and fallthrough serve the first variation 100%; experiment targeting flags are true; not bound to an experiment. |
| 9 | `rd-search-ranking-fixed-v1` | Enabled is `true`; variants are `control-updated=baseline`, `candidate-1-updated=semantic`, `candidate-2-updated=hybrid`, `candidate-updated=candidate`; rule is `User is in segment IsOneOf [segmentId]`; rule and fallthrough serve the first variation 100%; experiment targeting flags are true; not bound to an experiment. |
| 10 | `rd-kill-switch-fixed-v1` | Enabled is `false`; variants are `control-updated=false` and `treatment-updated=true`; rule is `User is in segment IsOneOf [segmentId]`; rule and fallthrough serve the first variation 100%; experiment targeting flags are true; not bound to an experiment. |

#### Public Evaluation And Insight Evidence

| Check | Where to look | Expected value |
| --- | --- | --- |
| Pre-seed evaluation count | Live report public-evaluation section | At least `29` evaluations: 10 flags for user 0, 10 flags for user 1, and 9 non-experiment flags for user 3 |
| Segment rule hits | Live report public-evaluation section | User `e2e-user-0003` evaluates every non-experiment flag to the first variation because that user is included in the generated segment |
| Seeded assignments | Live report insight/stats section | Logical assignments are `configured --users` for the experiment flag |
| Insight ingest | Live report step results, evaluation-service logs, or network traces | Preset events are posted to `POST /api/public/insight/track` without an error |
...
| Intent | Experiment detail | States that the test validates checkout activation and experiment analysis through FeatBit API evidence |
...
- Pre-seed public-evaluation counts and targeting verification counts
