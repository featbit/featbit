...
static async Task ExecuteAsync(FeatBitApiClient api, TestReport report, E2ERun run, E2EOptions options)
{
    ...
    await api.SendAsync(
        ...
        new
        {
            ...
            intent = "Validate that REST API, public evaluation, preset insight ingestion, and release-decision analysis agree end to end.",
            hypothesis,
    ...
    var primaryMetric = await UpsertReleaseDecisionMetricAsync(
        api,
        run.EnvId,
        run.PrimaryMetric,
        "Checkout Activation",
    ...
    var errorMetric = await UpsertReleaseDecisionMetricAsync(
        api,
        run.EnvId,
        run.ErrorMetric,
        "Checkout Error",
    ...
    var latencyMetric = await UpsertReleaseDecisionMetricAsync(
        api,
        run.EnvId,
        run.LatencyMetric,
        "Checkout Latency",
    ...
    var metricReuseExperiment = await api.SendAsync(
        ...
        "Create a second release-decision experiment that intentionally reuses the registered primary metric.");
    run.MetricReuseExperimentId = RequiredString(metricReuseExperiment, "id");

    var reusedMetrics = await api.SendAsync(
        HttpMethod.Put,
        $"/api/v1/envs/{run.EnvId}/release-decision/experiments/{run.MetricReuseExperimentId}/metrics",
        new
        {
            metricId = primaryMetricId,
            metricKey = run.PrimaryMetric,
    ...
    var reuseRun = await api.SendAsync(
        HttpMethod.Post,
        $"/api/v1/envs/{run.EnvId}/release-decision/experiments/{run.MetricReuseExperimentId}/runs",
        new { },
    ...
    await api.SendAsync(
        HttpMethod.Put,
        $"/api/v1/envs/{run.EnvId}/release-decision/experiments/{run.MetricReuseExperimentId}/runs/{run.MetricReuseRunId}/audience",
        new
        {
            method = "bayesian_ab",
    ...
    await api.SendAsync(
        HttpMethod.Put,
        $"/api/v1/envs/{run.EnvId}/release-decision/experiments/{run.MetricReuseExperimentId}/runs/{run.MetricReuseRunId}",
        new
        {
            status = "collecting",
    ...
    await api.SendAsync(
        HttpMethod.Put,
        $"/api/v1/envs/{run.EnvId}/release-decision/experiments/{run.MetricReuseExperimentId}/runs/{run.MetricReuseRunId}/observation-window",
        new
        {
            observationStart,
            observationEnd
...
static async Task<JsonNode?> UpsertReleaseDecisionMetricAsync(
    FeatBitApiClient api,
    string envId,
    string key,
    string name,
    ...
    string meaning)
{
    ...
    var existingMetrics = await api.SendAsync(
        HttpMethod.Get,
        $"/api/v1/envs/{envId}/release-decision/metrics?key={encodedKey}&pageIndex=0&pageSize=100",
        null,
        $"{stepName} lookup",
    ...
    if (existing != null)
    {
        ...
        metric = await api.SendAsync(
            HttpMethod.Put,
            $"/api/v1/envs/{envId}/release-decision/metrics/{metricId}",
            payload,
            $"{stepName} update",
            meaning);
    }
    else
    {
        metric = await api.SendAsync(
            HttpMethod.Post,
            $"/api/v1/envs/{envId}/release-decision/metrics",
            payload,
            $"{stepName} create",
            meaning);
    }
    ...
    if (!string.Equals(status, "active", StringComparison.OrdinalIgnoreCase))
    {
        throw new InvalidOperationException(
            $"Release-decision metric '{key}' exists with status '{status}', but experiment metric selection requires an active metric.");
    }
    ...
    TestReport report)
{
    ...
    var scenarioMetric = await UpsertReleaseDecisionMetricAsync(
        api,
        run.EnvId,
        metricEvent,
...
[
    ...
    new("GET", "/api/v1/envs/{envId}/release-decision/metrics"),
    new("POST", "/api/v1/envs/{envId}/release-decision/metrics"),
    new("PUT", "/api/v1/envs/{envId}/release-decision/metrics/{id}"),
...
static string BuildPlanMarkdown(string dataSetId, FlagSpec[] flags)
{
    ...
    sb.AppendLine("- Primary and guardrail metric event keys are registered as environment-level release-decision metrics, then selected onto experiments by metric id/key; the primary metric is intentionally selected by two experiments to verify shared catalog usage.");
    ...
    AppendPlanStep(sb, "5 update", "Fill intent/hypothesis", "PUT /api/v1/envs/{envId}/release-decision/experiments/{id}", "Persist intent, hypothesis, change, constraints, env secret, and event URL.");
    AppendPlanStep(sb, "6", "Register, select, and reuse metrics", "GET/POST/PUT /api/v1/envs/{envId}/release-decision/metrics; POST /release-decision/experiments; PUT /release-decision/experiments/{id}/metrics", "Register primary/guardrail metrics, select them for the main experiment, then select the primary metric again in a second experiment.");
    ...
    AppendPlanStep(sb, "10", "Traffic-assignment scenarios", "POST /release-decision/experiments; POST /release-decision/metrics; POST /runs; PUT /audience; POST /experiment-stats/query; POST /analyze", "Create one independent experiment and registered metric per traffic scenario, then verify stats reflect run traffic assignment.");
