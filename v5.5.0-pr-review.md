...
- [PR A] `infra/clickhouse/docker-entrypoint-initdb.d/v5.5.0-release-decision-insights.sql`
- [PR M] `modules/back-end/src/Infrastructure/Persistence/DbServiceCollectionExtensions.cs`
...
7. Open `infra/clickhouse/docker-entrypoint-initdb.d/v5.5.0-release-decision-insights.sql` [PR A]. Review the typed `release_decision_exposure_events` and `release_decision_metric_events` tables, their partitions/order keys, and why they mirror the Postgres/Mongo optimized event structures rather than the deleted legacy DAS tag columns.
...
| `v5.5.0-release-decision-insights.sql` | ingestion/storage | Kafka engine table, materialized views, and typed event tables for ClickHouse analytics. |
