...
- `modules/back-end/src/Api/Controllers/ExperimentStatsController.cs`
- `modules/back-end/src/Application/ExperimentStats/ExperimentStatsVm.cs`
- `modules/back-end/src/Application/ExperimentStats/QueryExperimentStats.cs`
- `modules/back-end/src/Application/Services/IExperimentStatsService.cs`
- `modules/back-end/src/Domain/ReleaseDecisions/ReleaseDecisionExposureEvent.cs`
...
- `modules/back-end/src/Infrastructure/Persistence/DbServiceCollectionExtensions.cs`
- `modules/back-end/src/Infrastructure/Persistence/EntityFrameworkCore/AppDbContext.cs`
- `modules/back-end/src/Infrastructure/Persistence/EntityFrameworkCore/Configurations/ReleaseDecisionExposureEventConfiguration.cs`
- `modules/back-end/src/Infrastructure/Persistence/EntityFrameworkCore/Configurations/ReleaseDecisionMetricEventConfiguration.cs`
- `modules/back-end/src/Infrastructure/Persistence/MongoDb/MongoDbClient.cs`
- `modules/back-end/src/Infrastructure/Services/EntityFrameworkCore/ReleaseDecisionExperimentService.cs`
- `modules/back-end/src/Infrastructure/Services/EntityFrameworkCore/ReleaseDecisionExperimentStatsService.cs`
- `modules/back-end/src/Infrastructure/Services/MongoDb/ReleaseDecisionExperimentService.cs`
...
1. Start at `modules/back-end/src/Api/Controllers/ExperimentStatsController.cs`. This is the direct HTTP stats endpoint file. Confirm `POST /api/v{version}/envs/{envId}/experiment-stats/query` copies route `envId` into `QueryExperimentStats.EnvId` before sending through MediatR.
2. Open `modules/back-end/src/Application/ExperimentStats/QueryExperimentStats.cs`. This is the stats request and handler file. Confirm the input fields are `EnvId`, `FlagKey`, `MetricEvent`, `StartDate`, `EndDate`, `MetricType`, and `MetricAgg`.
3. In the same `QueryExperimentStats.cs` file, review validation. Confirm dates must be `yyyy-MM-dd`, `EndDate >= StartDate`, metric type is limited to `binary|continuous`, and aggregation is limited to `once|count|sum|average`.
4. Still in `QueryExperimentStats.cs`, review `QueryExperimentStatsHandler`. Confirm the handler delegates directly to `IExperimentStatsService.QueryAsync(request)` and does not implement provider-specific query logic.
5. Open `modules/back-end/src/Application/Services/IExperimentStatsService.cs`. This is the stats service boundary file. Confirm the contract has a single `QueryAsync(QueryExperimentStats request)` method used by both direct stats queries and analysis.
6. Open `modules/back-end/src/Infrastructure/Persistence/DbServiceCollectionExtensions.cs`. This is the provider wiring file. Confirm Mongo registers `MongoDb.ReleaseDecisionExperimentStatsService` and Postgres registers `EntityFrameworkCore.ReleaseDecisionExperimentStatsService` for `IExperimentStatsService`.
7. Open `modules/back-end/src/Infrastructure/Services/EntityFrameworkCore/ReleaseDecisionExperimentStatsService.cs`. This is the Postgres aggregation implementation file. Review the date window conversion, `GetUserContributionExpression()`, and SQL CTE chain: `first_eval`, `metric_source`, `metric_events`, `user_totals`, final per-variant aggregate.
8. In the same Postgres stats service, confirm first exposure is selected with `DISTINCT ON (user_key)` ordered by `exposed_at`, metric rows are joined only when `metric_ts >= exposure_ts`, binary metrics force `once`, and output rows populate users, conversions, sum value, sum squares, conversion rate, and average value.
9. Open `modules/back-end/src/Infrastructure/Services/MongoDb/ReleaseDecisionExperimentStatsService.cs`. This is the Mongo aggregation implementation file. Review the equivalent path: load matching exposures, sort by `ExposedAt`, group to first exposure per user, load matching metric events for those users, filter metrics after exposure, aggregate user contributions, and return ordered variant rows.
10. Open `modules/back-end/src/Domain/ReleaseDecisions/ReleaseDecisionExposureEvent.cs`. This is the exposure source model file. Confirm stats queries depend on `EnvId`, `FlagKey`, `UserKey`, `VariationId`, `VariationValue`, and `ExposedAt`; `Properties` and `CreatedAt` are carried but not used by the current aggregation.
11. Open `modules/back-end/src/Domain/ReleaseDecisions/ReleaseDecisionMetricEvent.cs`. This is the metric source model file. Confirm stats queries depend on `EnvId`, `UserKey`, `EventName`, `EventType`, `NumericValue`, and `OccurredAt`; `Properties` and `CreatedAt` are carried but not used by the current aggregation.
12. Open `modules/back-end/src/Infrastructure/Persistence/EntityFrameworkCore/Configurations/ReleaseDecisionExposureEventConfiguration.cs`. This is the EF exposure mapping file. Confirm the table name is `release_decision_exposure_events`, column names match the SQL script, and indexes cover `(EnvId, FlagKey, ExposedAt)` and `(EnvId, UserKey, ExposedAt)`.
13. Open `modules/back-end/src/Infrastructure/Persistence/EntityFrameworkCore/Configurations/ReleaseDecisionMetricEventConfiguration.cs`. This is the EF metric mapping file. Confirm the table name is `release_decision_metric_events`, column names match the SQL script, and indexes cover `(EnvId, EventName, OccurredAt)` and `(EnvId, EventName, UserKey, OccurredAt)`.
14. Open `modules/back-end/src/Infrastructure/Persistence/EntityFrameworkCore/AppDbContext.cs`. This is the EF context file. Confirm `ReleaseDecisionExposureEventConfiguration` and `ReleaseDecisionMetricEventConfiguration` are applied.
15. Open `modules/back-end/src/Infrastructure/Persistence/MongoDb/MongoDbClient.cs`. This is the Mongo collection mapping file. Confirm `ReleaseDecisionExposureEvent` maps to `ReleaseDecisionExposureEvents` and `ReleaseDecisionMetricEvent` maps to `ReleaseDecisionMetricEvents`.
16. Open `modules/back-end/src/Infrastructure/Services/EntityFrameworkCore/ReleaseDecisionExperimentService.cs`. This is the Postgres analysis caller file. Confirm `AnalyzeRunAsync()` calls `statsService.QueryAsync(new QueryExperimentStats { ... })` for the primary metric and again for each guardrail metric.
17. Open `modules/back-end/src/Infrastructure/Services/MongoDb/ReleaseDecisionExperimentService.cs`. This is the Mongo analysis caller file. Confirm it uses the same `IExperimentStatsService` query contract for primary metric and guardrail stats before building analysis input.
18. Open `modules/back-end/src/Application/ExperimentStats/ExperimentStatsVm.cs`. This is the stats output file. Confirm the output carries `EnvId`, `FlagKey`, `MetricEvent`, `Window`, and `Variants`; each variant row carries `Variant`, `Users`, `Conversions`, `SumValue`, `SumSquares`, `ConversionRate`, and `AvgValue`.
19. Open `infra/postgresql/docker-entrypoint-initdb.d/v5.5.0.sql`. This is the database script file for Postgres storage. Confirm it creates the exposure and metric event tables plus indexes used by the CTE query.
20. In the same SQL script, review `release_decision_run_variant_stats`. Confirm it is an optional rollup cache table and not the source of truth for the current direct stats endpoint or server-side analysis path.
