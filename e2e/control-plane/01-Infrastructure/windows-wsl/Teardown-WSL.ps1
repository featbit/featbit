<#
.SYNOPSIS
    Tear down the FeatBit WSL deployment in layered steps.

.DESCRIPTION
    Reverses what Quickstart-WSL.ps1 sets up. Pick how deep to go:
      ...
      Full      — everything in Clusters, plus remove the FeatBit nginx
                  config, WSL /etc/hosts entries, wizard progress file,
    ...
    Scope note — NOT touched by any level:
      ...
      * Windows-side hosts entries — run Configure-WindowsHostAccess.ps1 -Remove
...
if (-not $IsLinux) {
    Write-Host ""
    Write-Host "  ✗ Teardown-WSL.ps1 must be run from inside a WSL2 Linux distro." -ForegroundColor Red
    Write-Host ""
    exit 1
}
...
@"
  ...
  Source code and deployment.env are NOT touched.
  Windows hosts entries: run Configure-WindowsHostAccess.ps1 -Remove on Windows.
"@
        }
    }
...
Write-Host ""
Write-Host "  FeatBit WSL Teardown — Level: $Level" -ForegroundColor White
Write-Host ""
...
if ($Level -eq "Full") {
    Write-Host "  Windows-side cleanup (run from Windows PowerShell):" -ForegroundColor White
    Write-Host "    pwsh -File `"C:\Users\$env:USER\featbit-quickstart\Configure-WindowsHostAccess.ps1`" -Remove" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  To redeploy later:" -ForegroundColor White
    Write-Host "    pwsh $PSScriptRoot/Quickstart-WSL.ps1 -Reset" -ForegroundColor Cyan
    Write-Host ""
