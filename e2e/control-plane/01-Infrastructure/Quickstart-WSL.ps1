<#
.SYNOPSIS
    Full quickstart wizard for FeatBit Pro — Windows with WSL2 (no Hyper-V).

.DESCRIPTION
    Guides you through every step to get a working two-cluster FeatBit Pro
    environment on Windows using WSL2 without requiring Hyper-V.  The script is
    resumable: completed phases are saved to .quickstart-state-wsl.json and
    ...
    works reliably.  WSL2 uses a Microsoft-customised kernel that may restrict
    iptables inside nested containers.  The hyperv-warning phase will explain
    your options before the deployment begins.
...
function Write-Banner  {
    ...
    Write-Host "  ╚═╝     ╚══════╝╚═╝  ╚═╝   ╚═╝   ╚═════╝ ╚═╝   ╚═╝   " -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  Quickstart Wizard — Windows + WSL2 (no Hyper-V)" -ForegroundColor White
...
function Invoke-SystemPrereqs([PSCustomObject]$State) {
    Write-Step "Phase 2 — System Prerequisites (WSL, Git)  [requires admin]"
    Request-Elevation -Reason "WSL installation"
    ...
    if ($wslInstalled) {
        ...
        if ($PSCmdlet.ShouldProcess("WSL", "Install")) {
            ...
            if ($LASTEXITCODE -ne 0) {
                Write-Warn "WSL install returned non-zero — may already be installed or require a reboot"
                ...
                Write-Info "    .\Quickstart-WSL.ps1"
                Save-State $State
                exit 0
            }
        }
        Write-Success "WSL installed (a reboot may be needed on first use)"
    }
...
function Invoke-DevTools([PSCustomObject]$State) {
    ...
    Write-Warn "Docker Desktop should be configured to use the WSL2 backend."
    Write-Info "This is the default on machines without Hyper-V.  To verify:"
    Write-Info "  Docker Desktop → Settings → General → 'Use the WSL 2 based engine' (should be checked)"
    Write-Info ""
...
function Invoke-HyperVWarning([PSCustomObject]$State) {
    Write-Step "Phase 4 — Cross-Cluster Networking Warning"
    ...
    if (-not $hvEnabled) {
        ...
        Write-Host "  │  ⚠  CROSS-CLUSTER NETWORKING LIMITATION                 │" -ForegroundColor Yellow
        Write-Host "  └─────────────────────────────────────────────────────────┘" -ForegroundColor Yellow
        Write-Host ""
        Write-Host "  Hyper-V is NOT enabled on this machine." -ForegroundColor Yellow
        Write-Host ""
        Write-Host "  FeatBit's two-cluster setup (west + east) uses iptables MASQUERADE" -ForegroundColor Gray
        Write-Host "  rules inside minikube's Docker containers to route traffic between" -ForegroundColor Gray
        Write-Host "  clusters.  This requires a full Linux kernel." -ForegroundColor Gray
        Write-Host ""
        Write-Host "  With Hyper-V:  Docker Desktop runs a complete MobyLinux VM — iptables" -ForegroundColor Gray
        Write-Host "                 works natively and cross-cluster pods can reach each other." -ForegroundColor Gray
        Write-Host ""
        Write-Host "  Without Hyper-V: Docker Desktop uses WSL2's Microsoft-customised kernel." -ForegroundColor Gray
        Write-Host "                 iptables inside nested containers may be restricted," -ForegroundColor Gray
        Write-Host "                 which will cause MongoDB replication (and other cross-" -ForegroundColor Gray
        Write-Host "                 cluster traffic) to fail." -ForegroundColor Gray
        Write-Host ""
        Write-Host "  You have two options:" -ForegroundColor White
        Write-Host "    A) Continue anyway — some users report success on newer WSL2 kernels." -ForegroundColor Gray
        Write-Host "       If you see cross-cluster connectivity errors, try option B." -ForegroundColor Gray
        Write-Host ""
        Write-Host "    B) Run the deployment from inside a WSL2 terminal — the Linux kernel" -ForegroundColor Gray
        Write-Host "       inside WSL2 supports iptables natively, so cross-cluster routing" -ForegroundColor Gray
        Write-Host "       works correctly.  This exits the wizard and prints instructions." -ForegroundColor Gray
        Write-Host ""

        $choice = Wait-UserChoice "How would you like to proceed?" @("a","b")

        if ($choice -eq "b") {
            Write-Host ""
            Write-Host "  ── WSL2 Terminal Instructions ────────────────────────────────" -ForegroundColor Cyan
            Write-Host ""
            Write-Host "  1. Open a WSL2 terminal:" -ForegroundColor White
            Write-Host "       wsl" -ForegroundColor Gray
            Write-Host ""
            Write-Host "  2. Install Linux prerequisites (example for Ubuntu/Debian):" -ForegroundColor White
            Write-Host "       sudo apt-get update" -ForegroundColor Gray
            Write-Host "       sudo apt-get install -y curl git docker.io" -ForegroundColor Gray
            Write-Host "       # Install minikube:" -ForegroundColor DarkGray
            Write-Host "       curl -Lo minikube https://storage.googleapis.com/minikube/releases/latest/minikube-linux-amd64" -ForegroundColor Gray
            Write-Host "       chmod +x minikube && sudo mv minikube /usr/local/bin/" -ForegroundColor Gray
            Write-Host "       # Install kubectl:" -ForegroundColor DarkGray
            Write-Host "       curl -LO https://dl.k8s.io/release/`$(curl -sL https://dl.k8s.io/release/stable.txt)/bin/linux/amd64/kubectl" -ForegroundColor Gray
            Write-Host "       chmod +x kubectl && sudo mv kubectl /usr/local/bin/" -ForegroundColor Gray
            Write-Host ""
            Write-Host "  3. Navigate to the repo and run the deployment:" -ForegroundColor White
            Write-Host "       cd /mnt/c/path/to/featbit/e2e/control-plane" -ForegroundColor Gray
            Write-Host "       # or clone fresh:" -ForegroundColor DarkGray
            Write-Host "       git clone https://github.com/wss-cadenwheeler/featbit ~/featbit" -ForegroundColor Gray
            Write-Host "       cd ~/featbit/e2e/control-plane" -ForegroundColor Gray
            Write-Host ""
            Write-Host "  4. Run the deployment scripts in order (Linux equivalents):" -ForegroundColor White
            Write-Host "       ./Initialize-LocalRegistry.sh       # or use the PS1 via pwsh if installed" -ForegroundColor Gray
            Write-Host "       ./Deploy-FeatBitClusters.ps1 -RecreateClusters -DeploymentMode Advanced -DatabaseProvider MongoDb" -ForegroundColor Gray
            Write-Host "       ./Initialize-MongoDBReplicaSet.ps1" -ForegroundColor Gray
            Write-Host ""
            Write-Host "  NOTE: PowerShell 7 is available in WSL2:  sudo snap install powershell --classic" -ForegroundColor Yellow
            Write-Host "        Once installed, run 'pwsh' in the WSL2 terminal to use .ps1 scripts." -ForegroundColor Yellow
            Write-Host ""
            Write-Info "Exiting wizard.  Run it again if you decide to continue on Windows instead."
            exit 0
        }

        Write-Warn "Continuing with WSL2 backend.  If cross-cluster errors occur during"
        Write-Warn "deploy-clusters or mongo-replica-set, revisit option B above."
        Write-Host ""
    } else {
        Write-Success "Hyper-V is enabled — cross-cluster networking will work correctly"
        Write-Info "Note: since Hyper-V is present, consider using Quickstart-HyperV.ps1 instead"
        Write-Info "for the most reliable experience."
    }

    Complete-Phase $State "hyperv-warning"
}

function Invoke-RepoSetup([PSCustomObject]$State) {
    Write-Step "Phase 5 — Repository Setup"
...
function Invoke-ProxyFirstRun([PSCustomObject]$State) {
    Write-Step "Phase 7 — Nginx Proxy First Run  [requires admin]"
    ...
    Write-Info "The proxy will be fully configured after the second run (Phase 9)."
    Write-Info ""
...
function Invoke-DeployClusters([PSCustomObject]$State) {
    Write-Step "Phase 8 — Deploy FeatBit Clusters  (~20 minutes)"
    ...
    if (-not (Test-HyperVEnabled)) {
        Write-Warn "Running without Hyper-V.  If you see cross-cluster connectivity errors"
        Write-Warn "in this phase, see the WSL2 terminal instructions from Phase 4."
        Write-Info ""
    }
...
function Invoke-ProxySecondRun([PSCustomObject]$State) {
    Write-Step "Phase 9 — Nginx Proxy Second Run  [requires admin]"
...
function Invoke-PortForwards([PSCustomObject]$State) {
    Write-Step "Phase 10 — Start Port Forwards"
...
function Invoke-MongoReplicaSet([PSCustomObject]$State) {
    Write-Step "Phase 11 — Initialize MongoDB Replica Set"
    ...
    if (-not (Test-HyperVEnabled)) {
        Write-Warn "Running without Hyper-V.  If MongoDB replica set initialization fails"
        Write-Warn "with connection errors, cross-cluster iptables routing is likely the cause."
        Write-Warn "See the WSL2 terminal instructions from Phase 4 (run -Reset, choose option B)."
        Write-Info ""
    }
