...
const nextConfig = {
  ...
  output: 'standalone',
  // FeatBit overlay: keep the native Node server SDK external to the server
  // bundle. It is a CommonJS package used only in server-side API routes, so it
  // should be required from node_modules at runtime rather than bundled. The
  // standalone Dockerfile copies node_modules, so the dependency is present.
  serverExternalPackages: ['@featbit/node-server-sdk'],
  compiler: {
