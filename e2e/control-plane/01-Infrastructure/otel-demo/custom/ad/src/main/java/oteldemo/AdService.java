...
public final class AdService {
  ...
  private void start() throws IOException {
    ...
    // Create a flagd instance with OpenTelemetry (kept for the existing adManualGc/adHighCpu/
    // adFailure flagd flags).
    ...
    // unconfigured or unreachable, every flag getter falls back to a safe default and the service
    // runs normally.
    FeatBitFlags.init();
  ...
  /**
   * Known ad categories. Used by the ad-category experiment flag: the configured category string
   * is resolved through {@link #valueOf(String)} WITHOUT a pre-check, so an unknown experiment
   * variant raises an IllegalArgumentException that surfaces to the gRPC caller (intentional
   * unknown-variant resiliency gap).
   */
  private enum AdCategory {
  ...
  private static class AdServiceImpl extends oteldemo.AdServiceGrpc.AdServiceImplBase {
    ...
    public void getAds(AdRequest req, StreamObserver<AdResponse> responseObserver) {
      ...
      try {
        ...
        // --- FeatBit flags (native Java SDK) -------------------------------------------------
        // Read all three flags up front and record them as span attributes for observability.
        boolean personalizationEnabled = FeatBitFlags.personalizationEnabled();
        int maxCount = FeatBitFlags.maxCount();
        String category = FeatBitFlags.category();
        span.setAttribute("app.ads.featbit.personalization_enabled", personalizationEnabled);
        span.setAttribute("app.ads.featbit.max_count", (long) maxCount);
        span.setAttribute("app.ads.featbit.category", category);

        span.setAttribute("app.ads.contextKeys", req.getContextKeysList().toString());
        span.setAttribute("app.ads.contextKeys.count", req.getContextKeysCount());

        // ad-category (experiment): when set to a non-"all" value, serve only that category.
        // The variant string is resolved via AdCategory.valueOf WITHOUT validation, so an
        // unknown variant throws IllegalArgumentException (intentional unknown-variant gap).
        // "all" (default) and known categories are safe.
        if (!FeatBitFlags.DEFAULT_CATEGORY.equals(category)) {
          AdCategory adCategory = AdCategory.valueOf(category.toUpperCase()); // unchecked -> throws on unknown
          logger.info("ad-category experiment active: serving category " + adCategory.key);
          allAds = new ArrayList<>(service.getAdsByCategory(adCategory.key));
          adRequestType = AdRequestType.TARGETED;
          adResponseType = AdResponseType.TARGETED;
        } else if (req.getContextKeysCount() > 0) {
        ...
        // ad-personalization-enabled (release): safe personalized-ad code path. When enabled and a
        // session is present we record personalization metadata; ad selection itself stays safe.
        if (personalizationEnabled && sessionId != null) {
          span.setAttribute("app.ads.personalized", true);
          span.addEvent(
              "ads.personalized",
              Attributes.of(AttributeKey.stringKey("session.id"), sessionId));
        } else {
          span.setAttribute("app.ads.personalized", false);
        }

        // ad-max-count (config): apply WITHOUT validation. A candidate pool backs the slice so
        // that the default (2) and other small counts are safe; a negative or too-large value
        // triggers a real subList IndexOutOfBoundsException/IllegalArgumentException surfaced to
        // the gRPC caller (intentional resiliency gap).
        List<Ad> servingPool = new ArrayList<>(allAds);
        for (Ad ad : adsMap.values()) {
          if (servingPool.size() >= 4) {
            break;
          }
          servingPool.add(ad);
        }
        allAds = new ArrayList<>(servingPool.subList(0, maxCount)); // no validation on maxCount
        ...
        // Misconfigured FeatBit flag values (bad ad-max-count -> subList out of range; unknown
        // ad-category -> AdCategory.valueOf) land here and are surfaced to the gRPC caller as an
        // INTERNAL error. This is the intentional resiliency gap: present-but-bad flag values are
        // NOT sanitized.
        span.addEvent(
            "Error", Attributes.of(AttributeKey.stringKey("exception.message"), String.valueOf(e.getMessage())));
        span.setStatus(StatusCode.ERROR);
        logger.log(Level.WARN, "GetAds Failed due to flag value: {}", e.toString());
