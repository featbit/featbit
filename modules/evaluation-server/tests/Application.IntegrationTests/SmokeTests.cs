...
public class SmokeTests
{
    ...
    public async Task StartupEndpoint_WhenRequested_IsMapped()
    {
        ...
        // The status depends on whether the test host's dependencies are reachable; what this pins
        // is that the route exists and is anonymous, so an orchestrator pointing a startup probe at
        // it gets a health verdict rather than a 404 that would be read as a crash-looping pod.
    ...
    public async Task DiagnosticsEndpoint_WhenRequested_ReturnsAStructuredJsonReport()
    {
        ...
        // status word, which cannot tell an operator WHICH datacenter or store is unhappy.
        Assert.Contains("\"status\"", body);
        Assert.Contains("\"entries\"", body);
        Assert.Contains("Store Availability", body);
    }
