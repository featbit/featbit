...
/// </para>
/// <para>
/// This service assigns itself a fresh consumer group per process
/// (<c>evaluation-server-{guid}</c>), set on the shared <see cref="ConsumerConfig"/> when
/// <see cref="KafkaMessageConsumer"/> is constructed. Two consequences follow. The group id is read
/// lazily here rather than copied at construction, so this reader cannot observe an empty group
/// because of DI resolution order. And immediately after startup the group has committed nothing,
/// so lag reads as unknown rather than as zero — which is the truth, and is why an absent topic is
...
public sealed class KafkaLagReader : IDisposable
{
    ...
    public KafkaLagReader(
        ConsumerConfig config,
        IConfiguration configuration,
        ILogger<KafkaLagReader> logger)
        : this(config, KafkaConsumerTopics.For(configuration), logger)
    {
    }
