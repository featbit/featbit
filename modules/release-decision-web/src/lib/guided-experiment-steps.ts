...
export const GUIDED_EXPERIMENT_STEPS: GuidedExperimentStep[] = [
  {
    ...
    agentPromptTemplate: [
      ...
      "",
      "Act as a release-decision workflow agent, not a batch job. If the experiment is blank or missing key decision fields, do not recap empty fields and do not persist placeholder text. Ask one short direct question to help the user clarify the experiment.",
  ...
  },
  {
    ...
    completionCriteria: [
      ...
      "Audience, rollout percentage, or run traffic sampling is defined for the run.",
    ...
    agentPromptTemplate: [
      ...
      "",
      "Act as a release-decision workflow agent. Help the user make the change reversible and decide who should see it first.",
      "Use FeatBit flag MCP tools when available to inspect existing flags, create missing flags after explicit approval, configure targeting after explicit approval, and enable/disable the toggle after explicit approval.",
  ...
  },
  {
    ...
    agentPromptTemplate: [
      ...
      "",
      "Act as a release-decision workflow agent. Help the user select one registered deciding metric, a small set of registered guardrails, the event instrumentation, the run window, and the run traffic sampling plan.",
      ...
      "Before updating experiment metrics, call featbit_release_decision_list_metrics and select existing metric keys. If the needed key is missing, ask the user to approve creating it with featbit_release_decision_create_metric. Then write the selection with featbit_release_decision_update_metrics using metricKey for the primary metric and guardrails as registered metric keys. Do not invent metric keys inside featbit_release_decision_update_metrics.",
      "Persist metric updates through featbit_release_decision_update_metrics. Configure experiment traffic assignment through featbit_release_decision_update_run_traffic, not old trafficPercent/trafficOffset fields. Pick analysisSamplingPlan includeRate from the actual exposure distribution in the run window; do not reuse a fixed split example after the live flag rollout changes.",
  ...
  },
  {
    ...
    agentPromptTemplate: [
      ...
      "",
      "Act as a release-decision workflow agent. Help the user decide whether the evidence supports CONTINUE, PAUSE, ROLLBACK CANDIDATE, or INCONCLUSIVE.",
