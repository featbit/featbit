...
export const GUIDED_EXPERIMENT_STEPS: GuidedExperimentStep[] = [
  ...
  },
  {
    ...
    agentPromptTemplate: [
      ...
      "When the primary metric name, event, type, aggregation, or guardrails are confirmed, write them with featbit_release_decision_update_metrics. metricName must be short, metricEvent must be a key with no spaces, and metricType/metricAgg must use the supported enums. Do not store the technical metric contract only as free text in featbit_release_decision_update_experiment.",
