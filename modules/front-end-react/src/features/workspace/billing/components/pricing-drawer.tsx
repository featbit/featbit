...
export function PricingDrawer({
  ...
  const currentFineGrained = subscription?.addOnFeatures?.includes(FINE_GRAINED_ACCESS) ?? false;
  ...
  const [planMau, setPlanMau] = useState<Record<PlanKey, number>>({
    ...
    enterprise: currentPlan === "enterprise" ? currentMau : planMeta.enterprise.includedMau
  });
  const [fineGrainedByPlan, setFineGrainedByPlan] = useState<Record<PlanKey, boolean>>({
    free: false,
    pro: false,
  ...
  const [enterpriseBillingCycle, setEnterpriseBillingCycle] = useState<BillingCycle>(
  ...
  const handleMauChange = (plan: PlanKey, mau: number) => setPlanMau((value) => ({ ...value, [plan]: mau }));
  const handleFineGrainedChange = (plan: PlanKey, checked: boolean) => setFineGrainedByPlan((value) => ({ ...value, [plan]: checked }));
          ...
          {intent === "upgrade" ? (
            <div className="grid gap-4 lg:grid-cols-2">
              <RecommendationCard
                ...
                setFineGrained={(checked) => handleFineGrainedChange(currentPlan, checked)}
            ...
            <>
              <div className="grid gap-4 lg:grid-cols-3">
                {(["free", "pro", "growth"] as const).map((key) => (
                  <PlanCard
                    ...
                    onFineGrainedChange={(checked) => handleFineGrainedChange(key, checked)}
              ...
              <EnterpriseRow
                ...
                onFineGrainedChange={(checked) => handleFineGrainedChange("enterprise", checked)}
...
function PlanCard({
  ...
  const canUpdateCurrent = isCurrent && (
  ...
  const actionLabel = isCurrent
    ? (canUpdateCurrent ? t("workspace.billing.actions.updatePlan") : t("workspace.billing.actions.currentPlan"))
  ...
  const addOnFeatures = supportsFineGrained && fineGrained ? [FINE_GRAINED_ACCESS] : [];
  const total = planTotal({ plan: planKey, billingCycle, mau: selectedMau, addOnFeatures });

  return (
    <Card className={cn("relative flex min-h-[31rem] flex-col overflow-hidden rounded-md p-4 shadow-none transition-shadow", isCurrent && "border-primary shadow-sm")}>
      {isCurrent ? <div className="absolute inset-x-0 top-0 h-1 bg-primary" /> : null}
      <div className="flex items-start justify-between">
        <h3 className="text-xl font-semibold">{t(meta.nameKey)}</h3>
        {isCurrent ? <Badge className="bg-primary px-2.5 py-0.5 text-primary-foreground shadow-sm">{t("workspace.billing.actions.currentPlan")}</Badge> : null}
      </div>
      <p className="mt-2 min-h-10 text-sm text-muted-foreground">{t(meta.descriptionKey)}</p>
      <div className="mt-1 text-2xl font-semibold">{formatMoneyPerCycle(total, billingCycle)}</div>
      {showMauSlider ? (
        <MauSliderSection
          className="mt-5"
        ...
        {supportsFineGrained ? (
          <label className="flex items-center gap-2 border-t pt-3 text-foreground">
            ...
            <span className="flex flex-1 items-center justify-between gap-2">
              <span className="whitespace-nowrap">{t("workspace.billing.drawer.fineGrainedAccess")}</span>
              <span className="whitespace-nowrap text-muted-foreground">{t("workspace.billing.drawer.addOnPrice", { amount: `$${FINE_GRAINED_ACCESS_PRICE}` })}</span>
            </span>
          </label>
        ) : null}
      </div>
      <Button className="mt-auto" disabled={isCurrent && !canUpdateCurrent} onClick={() => onAction({ plan: planKey, billingCycle, mau: selectedMau, addOnFeatures })}>
        {actionLabel}
      </Button>
    </Card>
  );
...
function EnterpriseRow({
  ...
  const total = planTotal({ plan: "enterprise", billingCycle: selectedBillingCycle, mau: selectedMau, addOnFeatures });
  const actionLabel = isCurrent
    ? (canUpdateCurrent ? t("workspace.billing.actions.updatePlan") : t("workspace.billing.actions.currentPlan"))
    : t("workspace.billing.drawer.upgradeEnterprise");

  return (
    <Card className={cn("relative grid overflow-hidden rounded-md p-4 shadow-none transition-shadow lg:grid-cols-[20rem_1fr] lg:items-start", isCurrent ? "gap-5 border-primary shadow-sm" : "gap-5")}>
      {isCurrent ? <div className="absolute inset-x-0 top-0 h-1 bg-primary" /> : null}
      <div className="flex h-full flex-col">
        <div className="flex flex-wrap items-center gap-2">
          {isCurrent ? <Badge className="bg-primary px-2.5 py-0.5 text-primary-foreground shadow-sm">{t("workspace.billing.actions.currentPlan")}</Badge> : null}
        </div>
        <h3 className={cn("text-xl font-semibold", isCurrent && "mt-3")}>{t(planMeta.enterprise.nameKey)}</h3>
        <p className="mt-1 text-sm text-muted-foreground">{t(planMeta.enterprise.descriptionKey)}</p>
        <div className="mt-1 text-2xl font-semibold">{formatMoneyPerCycle(total, selectedBillingCycle)}</div>
        <div className="mt-3 inline-flex w-fit overflow-hidden rounded-md border">
          ...
          <button
            type="button"
            className={cn(
              "cursor-pointer border-l px-2.5 py-1 text-xs transition-colors",
              selectedBillingCycle === "yearly" && "bg-primary text-primary-foreground"
            )}
        ...
        <Button className="mt-5 w-full" disabled={isCurrent && !canUpdateCurrent} onClick={() => onAction({ plan: "enterprise", billingCycle: selectedBillingCycle, mau: selectedMau, addOnFeatures })}>
          {actionLabel}
        </Button>
      </div>
  ...
  return (
    <div className={cn("border-y py-3", className)}>
      <div className="mb-3 flex items-center justify-between gap-3 text-xs text-muted-foreground">
        ...
        <strong className="font-semibold text-foreground">{formatMau(value)} MAU</strong>
      </div>
      <Slider min={min} max={max} step={10000} value={[value]} onValueChange={([nextValue]) => onChange(nextValue)} />
...
function PlanFeatureList({ planKey, columns = false }: { planKey: PlanKey; columns?: boolean }) {
  const { t } = useTranslation();

  return (
    <ul className={cn("space-y-2", columns && "grid gap-x-4 gap-y-2 space-y-0 sm:grid-cols-2")}>
...
function RecommendationCard({
  ...
  return (
    <Card className={cn("rounded-md p-4 shadow-none", highlighted && "border-blue-500 bg-blue-50/40 dark:bg-blue-950/20")}>
      ...
      <Slider className="mt-3" min={40000} max={300000} step={10000} value={[selectedMau]} onValueChange={([value]) => setSelectedMau(value)} />
