...
public partial class ClientConnectionMadeHandler
{
    public static partial class Log
    {
        [LoggerMessage(1, LogLevel.Information, "Handling connection made message: {Message}",
            EventName = "HandlingConnectionMade")]
        private static partial void HandlingConnectionMadeCore(ILogger logger, string message);

        /// <summary>
        /// Logs an inbound connection-made message. <c>ConnectionMessage</c> carries the client's SDK
        /// secret, so the body is logged in full with any embedded credential hashed
        /// (<c>docs/observability/index.md</c> section 7).
        /// </summary>
        public static void HandlingConnectionMade(ILogger logger, string message)
            => HandlingConnectionMadeCore(logger, Redaction.HideCredentials(message));
