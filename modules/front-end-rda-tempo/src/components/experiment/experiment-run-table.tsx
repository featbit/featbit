        ...
        <div className="flex items-center justify-between gap-3 rounded-md border border-dashed px-3 py-2.5 bg-muted/20">
          <div className="flex items-start gap-2 min-w-0">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              {hasDecision
                ? "A decision is already on file. Ask Codex to revisit it against the latest analysis, update this run, and suggest follow-ups."
                : "Ask Codex to refresh the latest data, evaluate the evidence, and write an actionable decision back to this run."}
            </p>
          </div>
          <Button
            variant="outline"
        ...
        {onOpenAgentPrompt && (
          <div className="flex items-center justify-between gap-3 rounded-md border border-dashed px-3 py-2.5 bg-muted/20">
            <div className="flex items-start gap-2 min-w-0">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                {hasDecision
                  ? "Ask Codex to revisit this run decision against the latest analysis and update the next action."
                  : "Ask Codex to evaluate the latest evidence and write an actionable decision for this run."}
              </p>
            </div>
            <Button
              variant="outline"
...
function buildDecisionPrompt({
  ...
  return `@featbit-experimentation ${experimentId} --stage decision --run-id ${run.id}`;
}
...
function AgentDecisionPromptDialog({
  ...
  const prompt = run ? buildDecisionPrompt({ experimentId, run }) : "";
  const value = prompt;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[86vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            Copy this command into Codex. The FeatBit experimentation skill
            reads the selected run and handles analysis and decision persistence
            through the configured MCP server.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="rounded-md border border-border/80 bg-muted/20 p-2.5">
            <div className="mb-2 flex items-center justify-between gap-3">
              <p className="text-xs text-muted-foreground">
                Paste this into your current Codex conversation.
              </p>
              <Button
                type="button"
                size="sm"
