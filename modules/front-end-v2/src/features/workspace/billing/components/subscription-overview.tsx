...
export function SubscriptionOverview({
  ...
  const subscriberSince =
    subscription?.createdAt ?? subscription?.subscriberSince
    ...
    <Card className="overflow-hidden rounded-md shadow-none">
            ...
            {subscription?.pendingDowngrade ? (
              <div className="mt-3 flex items-center gap-2 rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-800 dark:bg-blue-950/30 dark:text-blue-300">
                ...
                {t("workspace.billing.overview.scheduledDowngrade", {
                  plan: t(
                    planMeta[normalizePlan(subscription.pendingDowngrade.plan)]
                      .nameKey
                  ),
                })}
              </div>
      ...
      {!isFree ? (
        <div className="grid border-y bg-muted/10 md:grid-cols-3">
          <PeriodItem
            label={t("workspace.billing.overview.billingPeriod")}
            value={`${formatDate(billingPeriodStart, lang)} - ${formatDate(billingPeriodEnd, lang)}`}
          />
          <PeriodItem
            label={t("workspace.billing.overview.nextCharge")}
            value={formatDate(billingPeriodEnd, lang)}
          />
          <PeriodItem
            label={t("workspace.billing.overview.subscriberSince")}
            value={formatDate(subscriberSince, lang)}
          />
        </div>
      ) : null}
      ...
      <div className="grid gap-4 p-4 lg:grid-cols-2">
        <div className="rounded-md border p-5">
          <h3 className="font-semibold">
            {t("workspace.billing.overview.currentUsage")}
          </h3>
          ...
          <div className="mt-2 flex justify-between text-xs text-muted-foreground">
            <span>
              {t("workspace.billing.overview.usedOf", {
        ...
        <div className="rounded-md border p-5">
          <h3 className="font-semibold">
            {t("workspace.billing.overview.feeBreakdown")}
          </h3>
          <FeeRow
            label={t("workspace.billing.overview.planFee", {
              plan: t(plan.nameKey),
            })}
          ...
          <div className="mt-3 border-t pt-3">
            <FeeRow
              label={t("workspace.billing.overview.totalCharge")}
              value={t("workspace.billing.overview.perMonth", {
                amount: `$${total}`,
              })}
              strong
