...
export const planMeta = {
  free: {
    name: "Free",
    nameKey: "workspace.billing.plans.free.name",
    descriptionKey: "workspace.billing.plans.free.description",
    basePrice: 0,
    includedMau: 1000,
    monthlyPrice: 0,
  },
  pro: {
    name: "Pro",
    nameKey: "workspace.billing.plans.pro.name",
    descriptionKey: "workspace.billing.plans.pro.description",
    basePrice: 49,
    includedMau: 10000,
    monthlyPrice: 49,
  },
  growth: {
    name: "Growth",
    nameKey: "workspace.billing.plans.growth.name",
    descriptionKey: "workspace.billing.plans.growth.description",
    basePrice: 149,
    includedMau: 40000,
    monthlyPrice: 149,
  },
  enterprise: {
    name: "Enterprise",
    nameKey: "workspace.billing.plans.enterprise.name",
    descriptionKey: "workspace.billing.plans.enterprise.description",
    basePrice: 449,
    includedMau: 80000,
    monthlyPrice: 449,
  },
...
export function currentTotal(subscription?: BillingSubscription) {
  ...
  const base = subscription.unitAmount
    ? subscription.unitAmount / 100
    : plan.basePrice
  const addOn = subscription.addOnFeatures?.includes(FINE_GRAINED_ACCESS)
    ? 60
    : 0
  return base + addOn
}
...
export function usageStats(
  ...
  const purchased =
    subscription?.mau ??
    subscription?.baseMau ??
    planMeta[normalizePlan(subscription?.plan)].includedMau
...
export function planTotal(
  payload: Pick<SubscriptionChangePayload, "plan" | "mau" | "addOnFeatures"> &
    Partial<Pick<SubscriptionChangePayload, "billingCycle">>
) {
  const planKey = normalizePlan(payload.plan)
  const plan = planMeta[planKey]
  const isYearlyEnterprise =
    planKey === "enterprise" &&
    (payload.billingCycle === "yearly" || payload.billingCycle === "year")
  ...
  const addOnPrice = payload.addOnFeatures.includes(FINE_GRAINED_ACCESS)
    ? 60 * cycleMultiplier
    : 0
  const basePrice = isYearlyEnterprise
