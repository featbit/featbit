...
export function RelationshipPickerSheet({
  ...
  async function loadNextPage() {
    ...
    try {
      ...
      // Keep loaded options visible so scrolling can retry the next page.
    } finally {
      loadingMoreRef.current = false
      ...
      <SheetContent className="gap-0 p-0 data-[side=right]:w-[min(100vw,520px)] data-[side=right]:sm:max-w-[520px]">
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-hidden px-6 py-5">
          ...
          >
            ...
            <CommandList
              ref={listRef}
              className="max-h-56 flex-none [scrollbar-width:thin] [scrollbar-color:var(--border)_transparent] overflow-y-auto border-t p-1"
            >
              {loading ? (
