...
import type { RelationshipOption } from "../../team-api"
import {
  fetchRelationshipOptions,
  getCachedRelationshipOptions,
  type RelationshipOptionsLoader,
...
export function RelationshipPickerSheet({
  ...
  useEffect(() => {
    ...
    const cachedResult = getCachedRelationshipOptions(
      queryClient,
      cacheKey,
      query
    )
    if (cachedResult) {
      const applyCachedTimeout = window.setTimeout(() => {
        if (requestVersion.current !== version) return
        setOptions(cachedResult.items)
    ...
    const loadTimeout = window.setTimeout(
      () => {
        fetchRelationshipOptions(queryClient, cacheKey, query, loadOptions)
          .then((result) => {
            if (requestVersion.current !== version) return
  ...
  const cachedResult =
    filter === "all"
      ? getCachedRelationshipOptions(queryClient, cacheKey, query)
      : undefined
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden px-6 py-5">
          ...
          >
            ...
            <SelectionFilterTabs
              ...
              allLabel={t("iam.team.details.allFilter")}
              selectedLabel={(count) =>
                t("iam.team.details.selectedFilter", { count })
              }
