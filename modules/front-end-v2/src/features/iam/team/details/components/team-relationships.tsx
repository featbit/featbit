...
export function TeamRelationships({
  ...
  const permissions = useMemberPermissions(memberId)
    ...
    <>
      ...
      >
        <TabsList variant="line" className="flex min-w-max gap-8">
          <DetailTabTrigger
            value="permissions"
            label={t("iam.team.details.permissions")}
            count={permissions.items.length}
          />
          <DetailTabTrigger
            value="groups"
          ...
          <div className="flex items-center justify-between gap-4 py-5">
            <div className="relative w-80">
              ...
              <Input
                ...
                className="pl-9"
                onChange={(event) =>
                  relationships.setSearch(event.target.value)
                }
              />
            </div>
            {activeTab === "groups" ? (
