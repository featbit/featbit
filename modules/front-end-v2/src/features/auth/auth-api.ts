...
export async function loginByEmail(email: string, password: string) {
  return postIdentityLogin("/api/v1/identity/login-by-email", {
    email,
    password,
  })
}
...
export async function getSocialProviders() {
  ...
  const response = await fetch(
    apiUrl(`/api/v1/social/providers?${params.toString()}`)
  )

  const providers = unwrapEnvelope(
    await parseJsonResponse<ApiEnvelope<OAuthProvider[]>>(response)
  )
...
export async function getSsoPreCheck() {
  ...
  return unwrapEnvelope(
    await parseJsonResponse<ApiEnvelope<SsoPreCheck>>(response)
  )
}
