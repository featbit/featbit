    ...
    <div className="mt-4 flex flex-wrap items-center justify-between gap-4 text-sm text-muted-foreground">
      ...
      <div className="flex items-center gap-2">
        <Button
          ...
          aria-label={t("releaseDecision.metrics.previous")}
        ...
        <Button
          ...
          aria-label={t("releaseDecision.metrics.next")}
          onClick={() => onPageIndexChange(pageIndex + 1)}
        >
          <ChevronRight />
        </Button>
        ...
        >
          <SelectTrigger className="w-32">
            <SelectValue>
              {t("releaseDecision.metrics.perPage", { count: pageSize })}
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
            <SelectGroup>
              {[10, 20, 30].map((size) => (
                <SelectItem key={size} value={String(size)}>
                  {t("releaseDecision.metrics.perPage", { count: size })}
                </SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
