    ...
    () => [
      {
        ...
        cell: ({ row }) =>
          formatDate(row.original.billingDate ?? row.original.createdAt),
      },
      {
        accessorKey: "plan",
      ...
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold">
            {t("workspace.billing.invoices.title")}
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("workspace.billing.invoices.description")}
          </p>
        </div>
...
function InvoiceStatus({ status }: { status?: string }) {
  ...
  if (normalized === "paid") {
    return (
      <Badge
        variant="outline"
        className="border-emerald-300 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300"
      >
        {t("workspace.billing.invoices.paid")}
      </Badge>
    )
  }
  ...
  if (normalized === "overdue") {
    return (
      <Badge variant="destructive">
        {t("workspace.billing.invoices.overdue")}
      </Badge>
    )
  }
  return (
    <Badge variant="secondary">
      {status || t("workspace.billing.invoices.unknown")}
    </Badge>
  )
}
