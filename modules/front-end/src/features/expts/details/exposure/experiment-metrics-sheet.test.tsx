...
import { parseGuardrailMetrics, parsePrimaryMetric } from "./exposure-utils"
...
describe("editing experiment metrics", () => {
  it("restores saved selections outside the first catalog page", async () => {
    const primaryConfig: PrimaryMetricConfig = {
      metricId: "primary-id",
    ...
    const guardrailConfigs: GuardrailMetricConfig[] = [
      {
        metricId: "guardrail-id",
    ...
    const primary = parsePrimaryMetric(primaryConfig)
    const guardrailMetrics = parseGuardrailMetrics(guardrailConfigs)
