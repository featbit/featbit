          ...
          >
            {runs.map((run) => (
              <TabsTrigger
                ...
                className="h-11 min-w-32 flex-none justify-between gap-3 rounded-none border-0 border-r border-border px-4 font-normal text-foreground last:border-r-0 hover:bg-muted/40 focus-visible:z-20 data-active:bg-blue-50/60 data-active:shadow-none data-active:after:bottom-0 data-active:after:bg-blue-600 data-active:after:opacity-100 dark:data-active:bg-blue-950/25"
              >
                <code className="font-medium text-foreground">{run.slug}</code>
                {run.decision?.trim() ? (
                  <Badge variant="outline" className="font-normal">
                    {t(
                      `releaseDecision.experiments.detailsPage.measuring.decisions.${run.decision.trim().toLowerCase()}`,
                      { defaultValue: run.decision.trim().toUpperCase() }
                    )}
                  </Badge>
                ) : null}
              </TabsTrigger>
