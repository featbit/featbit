...
export class DoLoginComponent implements OnInit {
  ...
  async handleResponse(response) {
    ...
    if (!response.success) {
      this.message.error($localize `:@@login.incorrect-email-or-password:Email and/or password incorrect` );
      return;
    }

    await this.identityService.doLoginUser(response.data.token);
    this.message.success($localize `:@@login.login-success:Login with success`);
  }

  handleError(_) {
    this.isLogin = false;

    this.message.error($localize `:@@login.login-error:Error occurred, please contact the support.`);
  }
}
