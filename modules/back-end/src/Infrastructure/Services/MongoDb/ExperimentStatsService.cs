...
public class ExperimentStatsService(MongoDbClient mongoDb) : IExperimentStatsService
{
    public async Task<ExperimentStatsVm> QueryAsync(QueryExperimentStats request)
    {
        ...
        var metricDocs = await mongoDb.CollectionOf<ExperimentMetricEvent>()
            .Find(x =>
                x.EnvId == request.EnvId &&
    ...
    private async Task<ExperimentStatsVm> QueryAnalysisSamplingPlanAsync(QueryExperimentStats request)
    {
        ...
        if (request.RunId.HasValue && assignments.Length > 0)
        {
            ...
            var writes = assignments.Select(item =>
                new UpdateOneModel<ExperimentRunAssignment>(
                    Builders<ExperimentRunAssignment>.Filter.And(
                        Builders<ExperimentRunAssignment>.Filter.Eq(x => x.RunId, request.RunId.Value),
                        Builders<ExperimentRunAssignment>.Filter.Eq(x => x.AssignmentUnit, item.AllocationKey)),
                    Builders<ExperimentRunAssignment>.Update
                ...
                {
                    IsUpsert = true
                }).Cast<WriteModel<ExperimentRunAssignment>>().ToArray();

            await mongoDb.CollectionOf<ExperimentRunAssignment>().BulkWriteAsync(writes);
        }
        ...
        if (request.RunId.HasValue && assignments.Length > 0)
        {
            ...
            var writes = assignments.Select(item =>
                new UpdateOneModel<ExperimentRunAssignment>(
                    Builders<ExperimentRunAssignment>.Filter.And(
                        Builders<ExperimentRunAssignment>.Filter.Eq(x => x.RunId, request.RunId.Value),
                        Builders<ExperimentRunAssignment>.Filter.Eq(x => x.AllocationKey, item.AllocationKey)),
                    Builders<ExperimentRunAssignment>.Update
                        .SetOnInsert(x => x.Id, Guid.NewGuid())
                        .SetOnInsert(x => x.CreatedAt, now)
                        .Set(x => x.RunId, request.RunId.Value)
    ...
    private static IEnumerable<ExperimentExposureEvent> ApplyTrafficScope(
        QueryExperimentStats request,
        IEnumerable<ExperimentExposureEvent> exposures)
    {
    ...
    private static IEnumerable<ExperimentExposureEvent> ApplyMatchedVariantScope(
        QueryExperimentStats request,
        ExperimentExposureEvent[] firstEvaluations,
