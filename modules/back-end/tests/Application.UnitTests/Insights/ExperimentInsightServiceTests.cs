...
public class ExperimentInsightServiceTests
{
    [Fact]
    public void TryParseBuildsExposureEventFromFlagValueInsight()
    {
        var service = new ExperimentInsightService(null!);
        var id = Guid.NewGuid();
        ...
        Assert.True(success);
        var exposure = Assert.IsType<ExperimentExposureEvent>(insight);
    ...
    public void TryParseBuildsMetricEventFromTrackedInsight()
    {
        ...
        Assert.True(success);
        var metric = Assert.IsType<ExperimentMetricEvent>(insight);
