...
| Export gate | Registered **only when `ENABLE_OPENTELEMETRY` is exactly `true`**, matching the entrypoint's own comparison byte for byte — `True` and a whitespace-padded value leave the profiler off, so they must leave the sampler off too. The sampler exists to feed a gauge, so with export off the query is datastore load with no consumer, and a gate that spends money must never be more permissive than the gate that produces the signal. Observability is opt-in, and that has to include the cost it imposes on a production store rather than only the cost of shipping the data |
